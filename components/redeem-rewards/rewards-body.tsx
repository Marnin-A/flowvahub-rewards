		...
		<div className="grid gap-6 grid-cols-[repeat(auto-fill,minmax(300px,1fr))] mt-6">
			{rewards.map((reward) => {
				...
				return (
					<div
						key={reward.id}
						className={`border border-[#E9D4FF] bg-white rounded-[12px] p-6 shadow-[0_2px_8px_rgba(0,0,0,0.05)] relative overflow-hidden transition-all duration-200 ease-linear hover:translate-y-[-5px] hover:shadow-[0_6px_16px_rgba(0,0,0,0.1)] ${!isUnlocked ? "opacity-[0.7] cursor-not-allowed" : ""
							}`}
					>
						...
						<button
							disabled={!isUnlocked}
							className={`w-full font-semibold capitalize p-3 rounded-[8px] transition-all duration-300 ease-in-out ${isUnlocked
									? "bg-[#9013fe] text-white hover:bg-[#7c3aed]"
									: isComingSoon
