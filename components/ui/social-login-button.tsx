"use client";

import * as React from "react";
import Image from "next/image";
import { cn } from "@/lib/utils";
import { signInWithGoogle, signUpWithGoogle } from "@/lib/actions/auth";
import { Loader2 } from "lucide-react";

type SocialProvider = "google" | "facebook" | "apple" | "github";
type AuthMode = "signin" | "signup";

interface SocialLoginButtonProps
	extends React.ButtonHTMLAttributes<HTMLButtonElement> {
	provider: SocialProvider;
	label?: string;
	mode?: AuthMode;
	referralCode?: string | null;
}

const providerConfig: Record<
	SocialProvider,
	{ icon: string; defaultSignInLabel: string; defaultSignUpLabel: string }
> = {
	google: {
		icon: "data:image/svg+xml,%3csvg%20width='29'%20height='29'%20viewBox='0%200%2029%2029'%20fill='none'%20xmlns='http://www.w3.org/2000/svg'%3e%3cpath%20d='M27.1888%2014.7814C27.1888%2013.7382%2027.1024%2012.9769%2026.9155%2012.1875H14.7603V16.8959H21.8951C21.7513%2018.066%2020.9745%2019.8282%2019.2483%2021.0123L19.2241%2021.1699L23.0674%2024.0877L23.3336%2024.1137C25.779%2021.9005%2027.1888%2018.644%2027.1888%2014.7814Z'%20fill='%234285F4'/%3e%3cpath%20d='M14.7593%2027.1871C18.2548%2027.1871%2021.1893%2026.0592%2023.3327%2024.1139L19.2474%2021.0124C18.1541%2021.7595%2016.6868%2022.2811%2014.7593%2022.2811C11.3357%2022.2811%208.43002%2020.0679%207.39421%2017.0088L7.24238%2017.0214L3.24611%2020.0524L3.19385%2020.1947C5.32279%2024.3393%209.69579%2027.1871%2014.7593%2027.1871Z'%20fill='%2334A853'/%3e%3cpath%20d='M7.39501%2017.0095C7.1217%2016.2201%206.96353%2015.3742%206.96353%2014.5002C6.96353%2013.6261%207.1217%2012.7803%207.38063%2011.9909L7.37339%2011.8228L3.32703%208.74316L3.19464%208.80488C2.3172%2010.5248%201.81372%2012.4561%201.81372%2014.5002C1.81372%2016.5443%202.3172%2018.4756%203.19464%2020.1954L7.39501%2017.0095Z'%20fill='%23FBBC05'/%3e%3cpath%20d='M14.7594%206.7183C17.1904%206.7183%2018.8302%207.74739%2019.7653%208.60738L23.4191%205.11125C21.1751%203.06716%2018.2549%201.8125%2014.7594%201.8125C9.69583%201.8125%205.3228%204.66012%203.19385%208.80467L7.37985%2011.9907C8.43004%208.93159%2011.3358%206.7183%2014.7594%206.7183Z'%20fill='%23EB4335'/%3e%3c/svg%3e",
		defaultSignInLabel: "Sign in with Google",
		defaultSignUpLabel: "Sign up with Google",
	},
	facebook: {
		icon: "",
		defaultSignInLabel: "Sign in with Facebook",
		defaultSignUpLabel: "Sign up with Facebook",
	},
	apple: {
		icon: "",
		defaultSignInLabel: "Sign in with Apple",
		defaultSignUpLabel: "Sign up with Apple",
	},
	github: {
		icon: "",
		defaultSignInLabel: "Sign in with GitHub",
		defaultSignUpLabel: "Sign up with GitHub",
	},
};

function SocialLoginButton({
	provider,
	label,
	mode = "signin",
	referralCode,
	className,
	...props
}: SocialLoginButtonProps) {
	const [isLoading, setIsLoading] = React.useState(false);
	const config = providerConfig[provider];
	const displayLabel = label || (mode === "signup" ? config.defaultSignUpLabel : config.defaultSignInLabel);

	const handleClick = async () => {
		if (provider === "google") {
			setIsLoading(true);
			if (mode === "signup") {
				await signUpWithGoogle(referralCode || undefined);
			} else {
				await signInWithGoogle();
			}
			setIsLoading(false);
		}
	};

	return (
		<button
			type="button"
			disabled={isLoading}
			onClick={handleClick}
			className={cn(
				"border py-[11px] px-[14px] text-sm w-full gap-2 text-[#111827] border-[#EDE9FE] rounded-md hover:bg-[#F5F3FF] transition-colors flex items-center justify-center relative disabled:opacity-50 disabled:cursor-not-allowed",
				className
			)}
			{...props}
		>
			{isLoading ? (
				<>
					<Loader2 className="w-6 h-6 animate-spin" />
					<span>Connecting...</span>
				</>
			) : (
				<>
					{config.icon && (
						<Image
							src={config.icon}
							alt={provider}
							width={24}
							height={24}
							className="w-6 h-6"
						/>
					)}
					<span>{displayLabel}</span>
				</>
			)}
		</button>
	);
}

export { SocialLoginButton };
export type { SocialProvider, AuthMode };

