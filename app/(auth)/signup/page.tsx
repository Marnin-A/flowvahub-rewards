...
import { signupSchema, validatePassword, type SignupFormData } from '@/lib/schemas/signup-schema';
    ...
    <div className="mt-2 mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
      ...
      <ul className="space-y-1">
        {validations.map(rule => (
          ...
          >
            {rule.isValid ? <Check className="w-3 h-3" /> : <Circle className="w-3 h-3" />}
            {rule.message}
          </li>
        ))}
      </ul>
...
function SignupForm() {
  ...
  const onSubmit = async (data: SignupFormData) => {
    setError(null);
    setSuccess(false);
    const result = await signUp(data.email, data.password, referralCode || undefined);
    if (result?.error) {
    ...
    <div className="flex justify-center w-full max-w-[420px]">
      <div className="w-full shadow-[0_4px_6px_rgba(0,0,0,0.1)] py-[30px] px-[20px] lg:p-[40px] bg-white rounded-[10px] animate-fadeIn h-fit">
        ...
        <div className="w-full">
          <form className="w-full text-[#111827]" onSubmit={handleSubmit(onSubmit)}>
            <FormInput
              label="Email"
              type="email"
            ...
            {password && password.length > 0 && <PasswordRequirements password={password} />}
          ...
          <div className="text-center mt-5 text-sm">
            <p className="text-[#6B7280]">
              ...
              <Link href="/login" className="text-primary-purple no-underline font-medium hover:underline">
                Log In
              </Link>
            </p>
          </div>
